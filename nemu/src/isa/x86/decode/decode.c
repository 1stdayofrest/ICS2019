...
#define make_DopHelper(name)                                                   \
  void concat(decode_op_, name)(vaddr_t * pc, Operand * op, bool load_val)
...
static inline make_DopHelper(r) {
  ...
  if (load_val) {                 //函数中将带有一个load_val参数,
                  //用于控制是否需要将该操作数读出到全局译码信息decinfo供后续使用.
 ...
 * Sw
 */
static inline void decode_op_rm(vaddr_t *pc, Operand *rm, bool load_rm_val,
 ...
 * Ev <- Gv
 */
make_DHelper(G2E) { decode_op_rm(pc, id_dest, true, id_src, true); }

make_DHelper(mov_G2E) { decode_op_rm(pc, id_dest, false, id_src, true); }

/* Gb <- Eb
 * Gv <- Ev
 */
make_DHelper(E2G) { decode_op_rm(pc, id_src, true, id_dest, true); }

make_DHelper(mov_E2G) { decode_op_rm(pc, id_src, true, id_dest, false); }

make_DHelper(lea_M2G) { decode_op_rm(pc, id_src, false, id_dest, false); }
...
make_DHelper(I) { decode_op_I(pc, id_dest, true); }

make_DHelper(r) { decode_op_r(pc, id_dest, true); }

make_DHelper(E) { decode_op_rm(pc, id_dest, true, NULL, false); }

make_DHelper(setcc_E) { decode_op_rm(pc, id_dest, false, NULL, false); }

make_DHelper(gp7_E) { decode_op_rm(pc, id_dest, false, NULL, false); }

/* used by test in group3 */
make_DHelper(test_I) { decode_op_I(pc, id_src, true); }
...
make_DHelper(J) { //跳转指令
  decode_op_SI(pc, id_dest, false);//有符号操作数译码
  // the target address can be computed in the decode stage
  decinfo.jmp_pc = id_dest->simm + *pc; //为什么是加这个呢
}

make_DHelper(push_SI) { decode_op_SI(pc, id_dest, true); }
